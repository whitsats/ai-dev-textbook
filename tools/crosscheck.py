...
- `owner`：**出处类**——这条纪律/结论是哪一章**定义**的（`### 离线当门，真机报数`
  在 3.9，就要写 3.9——十二处引用曾把它记成 3.10，2026-09-25 已统一，见第一条）；
...
class Site:
    ...
    - `role`：`断言` = 这个值要与权威值比；`旁证` = 只核它在不在（不参与判定，
      例如同一句里汉字写的篇数——汉字是引文、不是断言，沿用 5.9 那一轮的约定）；
...
ENTRIES: tuple[Entry, ...] = (
    Entry(
        ...
        verdict="一致",
        divergence=(
            "**（已统一，2026-09-25）**这条纪律的**定义在 3.9.6**（`### 离线当门，真机报数`）；"
            "它是从 3.10 的端到端验收**开始成为整章的验收形态**的。原先十二处引用只写了 3.10——"
            "六处「（3.10 起）」，另六处是前置知识里的「3.10（组装与验收。那条「离线当门、真机报数」"
            "的纪律…」。**读者按「3.10 起」回溯，翻到的是 3.10.5 的两条读数（离线的读数、"
            "真机的读数），找不到那条纪律的定义**——而 3.9 才是它被写下来的地方。"
            "现在十二处一律**先写定义章、再写用途**。"),
        fix=(
            "已按方向一落地：把「定义」与「用途」拆成两截写——`3.9` 管「它在哪一章被定义下来」，"
            "`3.10` 管「它从哪一章起成为整章验收」。六处「（3.10 起）」改成"
            "「（3.9 定，3.10 起成为整章验收）」（5.3 那句是叙述体，写成「3.9 定下、3.10 起沿用」），"
            "六处前置知识把 3.9 与 3.10 并列成「3.9、3.10（…）」。"
            "**这也是这一格能对上的条件**：断言句式抓的是定义章（`(\d+\.\d+)\s*定[下，]`），"
            "不是句子里的第一个章号——改稿时把两个章号写反，`--check` 会当场报分歧。"
            "六处前置知识行原先登记为「旁证」，这一趟随正文一起升成断言：它们现在真的在说"
            "「出处是 3.9」。字数漂移按 8.7 的记账处补进各章的【事后校准】注，同一趟进提交。"),
        # 句式**不写死那个值**（`(\d+\.\d+)` 而不是 `(3\.9)`）：写死的话，
        # 正文一改，「清单与正文不符」就会被误报成「清单过期」——两种病、两句药，
        # 分不开的话拿到报错还得自己去比一遍。
        sites=(
            Site("5.3", r"(\d+\.\d+)\s*定下", "3.9"),
            Site("7.1", r"（(\d+\.\d+)\s*定，", "3.9"),
            Site("7.2", r"（(\d+\.\d+)\s*定，", "3.9"),
            Site("8.1", r"（(\d+\.\d+)\s*定，", "3.9"),
            Site("8.2", r"（(\d+\.\d+)\s*定，", "3.9"),
            Site("8.3", r"（(\d+\.\d+)\s*定，", "3.9"),
            # 六处前置知识行：原先只记「3.10（组装与验收…」，现在并列写 3.9 与 3.10，
            # 抓的是**第一个**章号（定义章）。4.1 的写法与第 8 篇那五处不同（它用
            # 「第 3.9 与 3.10 节的端到端验收与…」），所以句式单独一条。
            Site("4.1", r"第 (\d+\.\d+) 与 \d+\.\d+ 节的端到端验收与「离线当门、真机报数」", "3.9"),
            Site("8.1", r"(\d+\.\d+)、\d+\.\d+（组装与验收。那条「离线当门、真机报数」", "3.9"),
            Site("8.2", r"(\d+\.\d+)、\d+\.\d+（组装与验收。那条「离线当门、真机报数」", "3.9"),
            Site("8.3", r"(\d+\.\d+)、\d+\.\d+（组装与验收。那条「离线当门、真机报数」", "3.9"),
            Site("8.4", r"(\d+\.\d+)、\d+\.\d+（组装与验收：那条「离线当门、真机报数」", "3.9"),
            Site("8.5", r"(\d+\.\d+)、\d+\.\d+（组装与验收：那条「离线当门、真机报数」", "3.9"),
        ),
        # 两份普查各守一族：第一族是那五句「（3.9 定，3.10 起成为整章验收）」＋ 5.3 的叙述体，
        # 第二族是第 8 篇那五处前置知识行（4.1 那一处的写法是独一份，不设普查）。
        # 6.3 那句「**超时**：3.10 起，本树每一段真机读数都带着它的时限」是**另一条**结论
        # （超时纪律），两族句式都不会顺手把它算进来。
        census=((r"3\.9\s*定[下，]", 6), (r"3\.9、3\.10（组装与验收[。：]", 5)),
    ),
    Entry(
        key="前N篇",
        kind="计数",
        ...
        verdict="一致",
        divergence=(
            "**（已统一，2026-09-25）**同一个句式原先在三章里说的是三种东西，"
            "**没有一处能与盘上对上**：6.1 写「前三篇 41 章」（41 不属于任何口径）；"
            ...
            "坏的只是跨篇这一种。"),
        fix=(
            "已按口径一落地：口径写进 `STYLE` 第二节，与「前 N 章」那一条并列——"
            "**N ＝ 本篇号 − 1，不含第 0 篇导论；M ＝ 第 1–N 篇的章数之和**。"
            "三处按它重算成 45／49／54，句式本身不动（它与篇内那三章已经成立的写法同构，"
            "而本书此前没有「第 A–B 篇」这种写法的先例）。**N 由旁证格守着**："
            "`前([一二三四五六七八九十]+)篇` 验的正是「N ＝ 本篇号 − 1」这条口径本身，"
            "而不只是那个数——写错 N 或 M 都会红。下一章再写这句时照写。"),
        sites=(
            Site("6.1", r"前五篇\s*([\d,]+)\s*章", "45", scope="span:1-5"),
            Site("7.1", r"前六篇\s*([\d,]+)\s*章", "49", scope="span:1-6"),
            Site("8.1", r"前面七篇\s*([\d,]+)\s*章", "54", scope="span:1-7"),
            Site("6.1", r"前([一二三四五六七八九十]+)篇\s*[\d,]+\s*章", "五", role="旁证"),
...
#: `断言格` 这一趟随六处前置知识行从旁证升成断言而抬高（12 ＋ 3 ＝ 15）：
#: 下限跟着实况走，降下来就说明有格子从「要核的」漂回了「只核在不在」。
FLOORS: dict[str, int] = {"条目": 2, "格": 15, "断言格": 15}
...
def render(facts: Facts, entries: tuple[Entry, ...] = ENTRIES) -> str:
    ...
    out.append("> 「前 N 篇 M 章」这类句子写错时会看得很像真的：`52` 确实是一段真实的章数之和"
               ...
               "——2026-09-25 之前，三处就是这么错的。")
    out.append("> 这就是为什么那一格必须由盘上现算，而不是由同一句话里的另一个数推出来。"
               "口径见 `STYLE` 第二节：**N ＝ 本篇号 − 1**（不含第 0 篇导论）、"
               "M ＝ 第 1–N 篇的章数之和。")
    out.append("")
    ...
    out.append("")
    # 这句在两个状态下要说两句不同的话：还剩「待改」时提醒「修好要同笔改判定」，
    # 一条不剩时提醒的是相反方向的错——把已经统一的再挂回「待改」也会红。
    left = sum(1 for e in entries if e.verdict == "待改")
    tail = ("待改的那几条修好之后，**判定与建议改法要在同一笔提交里改**，否则 `--check` 两边都红。"
            if left else
            "没有挂着「待改」的条目；下一次正文再漂，`--check` 会把判定从「一致」翻回「分歧」"
            "——两个方向都要同笔改。")
    out.append(f"> **判定分布**：待改 {left} 条、一致 {len(entries) - left} 条。{tail}")
    out.append("")
    return "\n".join(out) + "\n"
