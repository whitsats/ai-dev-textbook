...
SPECS: tuple[TreeSpec, ...] = (
    ...
    TreeSpec(book=BOOK_V7, tree=TREE_V7, label="（第 7 篇）", marker="离线自检通过",
             ...
             # 下限**逐个量过再写**（建树当天量的值）：1 章／**3 个 `app/` 模块**
             # （`suite` / `metrics` / `gate`；`__init__` 与前面四棵树一样不计）／
             # 3 个测试模块／正文里 **8 个行数格**（交付物清单八行）与
             # **6 个条数格**（33 条夹具印两处 ＋ 12／11／19 条用例）／
             # **13 个标了命令的输出块**（十二组读数块 ＋ 夹具那一行）。
             min_chapters=1, min_modules=3, min_tests=3,
             min_line_cells=8, min_claim_cells=6, min_output_blocks=13),
