...
PLAN = [
    ...
    #     2.4 事前重估：6,000 → 14,000，素材权重 0.8 → 0.67（写作前改，两项都有实测依据）。
    #     2.4 事后校准：实写 15,301（汉字 11,836 + 代码 231 行），按实测校准 14,000 → 15,000。
    #       归因：2.4.9 原计划只是一段脚本，实际长成「三份夹具 + 三版对照 + 输出」一整节，
    #       并补了一手报告出处；教训是**可执行清单类内容的输入夹具也是正文**，估篇幅要算进去。
    #     ① 篇幅：原值同样是「素材量 × 比例」倒推的。按 8.7 式子：11 小节
    #        （9 长 × 900 + 2 短 × 450）+ 约 235 行示例/脚本/配置 × 15 + 固定段 ≈ 14,000。
    #     ② 权重：这份素材是**视频课件大纲**，实测构成是——课程旁白 1,799 汉字
    #        （「简介：讲解…」「掌握 X」，不可作散文素材）、文末团队规范 4,076、
    #        其余操作步骤与概念 9,694。按“本章实际用到的那部分”量：操作 0.9、规范 0.4
    #        → 0.67，散文素材 10,431，比值 0.75（偏薄）。原记 0.8 把课程旁白也当素材了。
    ("2", "2.4", "AI 编辑器与 Vibe Coding 全栈开发", 15000,
     [("file", "02-AI编程工具链/Vibe-Coding与AI编辑器全栈开发.md", 0.67)],
     "课件大纲，散文素材约 10,431 汉字"),
