...
#: 下限跟着实跑走，不是一次写死的：**四个数都要等于当前的实得值**。
#: 2026-09-22 把它们逐个核对了一遍，两个已经落在后面——「门脚本 16（实得 18）」与
#: 「复算入口 7（实得 11）」：下限跟在后面时，它守的不再是「有没有少」而是那句
#: 「实得 ≥ 一个很久以前的数」，而失去两支工具它一声也不会哨。
_FLOOR = {"树": 6, "入口脚本": 29, "门脚本": 18, "复算入口": 11}
